...
def otp_keyboard(otp: Optional[str], full_msg: str = "",
    ...
    if for_group:
        ...
        # Show only emoji + bold dots/stars - when clicked, OTP is copied
        if clean:
            # Simple button: emoji + stars, click to copy OTP
            rows.append([{"text": f"🔐 <b>{'★' * 4}</b>", "copy_text": {"text": clean}, "style": "success"}])
        link_row = []
