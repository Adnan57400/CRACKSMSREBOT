...
def build_otp_msg(header: str, count_badge: str, clean: str,
    ...
    # ── T0: SAMI OTP (from screenshot) ──── ENHANCED DESIGN ─────────
    if t == 0:
        if for_group:
...
def add_dynamic_design_elements(msg_text: str, theme_id: int) -> str:
    ...
    anim = animations.get(theme_id % 30, "✨")
...
def main_menu_kb() -> InlineKeyboardMarkup:
    """Professional main menu with colored buttons and all features."""
    rows = [
        [btn("🔥  Get Number",   cbd="buy_menu",        style="primary"),
         btn("👤  My Profile",   cbd="profile",         style="success")],
        [btn("📊  My Stats",      cbd="mystats",         style="primary"),
         btn("📜  My History",    cbd="myhistory",       style="success")],
        [btn("�  My OTPs",       cbd="my_otps",         style="danger"),
         btn("💎  Premium",       cbd="premium_menu",    style="success")],
        [btn("📈  Analytics",     cbd="analytics",       style="primary"),
         btn("🤖  Create My Bot", cbd="create_bot_menu", style="success")],
        [btn("⚙️   Settings",       cbd="user_settings",   style="primary"),
         btn("📚  Tutorials",     cbd="tutorials",       style="danger")],
    ]
    link_row = []
    if CHANNEL_LINK:
...
def admin_main_kb(perms: list, is_sup: bool) -> dict:
    ...
    r = []
    if is_sup: r.append(btn("👤  Users",       cbd="admin_users",        style="success"))
    if r: rows.append(r)
    ...
    # ADMINS & PERMISSIONS
    r = []
    if is_sup: r.append(btn("👥  Admins",        cbd="admin_manage_admins", style="danger"))
    if is_sup: r.append(btn("🔐  Permissions",   cbd="admin_perms",         style="success"))
    if r: rows.append(r)
    
    # TUTORIALS & SETTINGS
    r = []
    if is_sup: r.append(btn("📚  Tutorials",     cbd="admin_tutorials",     style="primary"))
    r.append(btn("⚙️  Settings",                  cbd="admin_settings",      style="primary"))
    if r: rows.append(r)
    
    # FETCH SMS & CHILD BOTS
    r = []
    if "manage_panels" in perms: r.append(btn("📡  Fetch SMS",  cbd="admin_fetch_sms",   style="success"))
    if is_sup:                   r.append(btn("🤖  Child Bots", cbd="admin_bots",        style="danger"))
    if r: rows.append(r)
    
    # SYSTEM (full-width button - important)
    r = []
    if is_sup: r.append(btn("🔧  System",        cbd="admin_system",        style="primary"))
    if r: rows.append(r)
    ...
    if uid in CREATE_BOT_STATES:
        ...
        if step == "get_group_id_panel":
            ...
            summary = (
                ...
                f"� Plan: {state.get('tier','free').upper()}\n"
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("approvebot_"):
        ...
        try:
            ...
            config = {
                "name": req.get("bot_name", "Unnamed Bot"),
                "bot_username": req.get("bot_username", "?"),
                "admin_ids": [req.get("uid", 0)],
                "channel_link": req.get("channel_link", CHANNEL_LINK),
                "otp_group_link": req.get("otp_group_link", OTP_GROUP_LINK),
            }
            
            folder = bm.create_bot_folder(bid, config)
            bm.register_bot(bid, config, folder)
...
async def post_init(application):
    ...
    logger.info(f"📊 OTP GUI Theme: #{OTP_GUI_THEME % 30}")
    logger.info("=" * 60)
