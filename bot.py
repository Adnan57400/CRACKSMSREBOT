...
def hide_otp_in_message(msg_text: str, clean_otp: str = "") -> str:
    ...
    privacy_placeholder = "🔐 <b>TAP BUTTON BELOW TO REVEAL</b>"
...
def admin_main_kb(perms: list, is_sup: bool) -> dict:
    """Professional admin menu with color-coded buttons by function."""
    rows = []
    # NUMBERS & BROADCAST (Primary/Success)
    r1 = []
    if "manage_files" in perms: r1.append(btn("📂  Numbers",    cbd="admin_numbers",      style="primary"))
    if "broadcast"    in perms: r1.append(btn("📢  Broadcast",  cbd="admin_broadcast",    style="danger"))
    if r1: rows.append(r1)
    # STATS & USERS (Success/Primary)
    r2 = []
    if "view_stats" in perms: r2.append(btn("📊  Statistics", cbd="admin_stats_menu",   style="success"))
    if "view_stats" in perms: r2.append(btn("📈  Advanced",    cbd="advanced_analytics", style="primary"))
    if is_sup:                r2.append(btn("👤  Users",       cbd="admin_users",        style="success"))
    if r2: rows.append(r2)
    # PANELS & LOGS (Primary/Success)
    r3 = []
    if "manage_panels" in perms: r3.append(btn("🔌  Panels",     cbd="admin_panel_manager",style="primary"))
    if "manage_logs"   in perms: r3.append(btn("📋  Log Groups", cbd="admin_manage_logs", style="primary"))
    if "manage_logs"   in perms: r3.append(btn("🗑  Clear Logs",  cbd="clear_all_logs",   style="danger"))
    if r3: rows.append(r3)
    # ADMINS & SETTINGS (Danger/Success)
    r4 = []
    if is_sup: r4.append(btn("👥  Admins",        cbd="admin_manage_admins", style="danger"))
    if is_sup: r4.append(btn("🔐  Permissions",   cbd="admin_perms",         style="success"))
    if is_sup: r4.append(btn("📚  Tutorials",     cbd="admin_tutorials",     style="primary"))
    r4.append(btn("⚙️  Settings",                  cbd="admin_settings",      style="primary"))
    rows.append(r4)
    # SMS & BOTS (Success/Primary)
    r5 = []
    if "manage_panels" in perms: r5.append(btn("📡  Fetch SMS",  cbd="admin_fetch",       style="success"))
    if is_sup:                   r5.append(btn("🤖  Child Bots", cbd="admin_bots",        style="danger"))
    if is_sup:                   r5.append(btn("🔧  System",     cbd="admin_system",      style="primary"))
