...
#  Features: Admin Panel • Multi-panel • Child Bots • 30 OTP Themes
...
# ── OTP GUI Theme ─────────────────────────────────────────────────
# 40 premium designs. Only super-admins can change. Clamp to index %40.
OTP_GUI_THEME = 0   # 0-39
...
_THEME_NAMES = {
    ...
    30: "💬 WhatsApp",               31: "📲 SMS Pro",
    32: "📧 Email",                  33: "📸 Instagram",
    34: "🐦 X/Twitter",              35: "🔥 Fire",
    36: "🌟 Elite",                  37: "📱 Voice",
    38: "✉️ Secure",                 39: "🛡️ Shield",
}
...
def build_otp_msg(header: str, count_badge: str, clean: str,
    ...
    # ── T0: CRACK──── ENHANCED DESIGN ─────────
    if t == 0:
        if for_group:
            # ✅ New clean format: Country | Service | Masked Number | Copyright
            masked_num = f"{num_full[:num_full.find('•')+1]}••{num_full[-4:]}" if '•' in num_full else f"{num_full[:-4]}••{num_full[-4:]}"
            return (f"🌍 {aflag} {region} | 📱 {svc} | 🗣 <i>Auto</i>\n"
                    f"📞 {masked_num}\n"
                    f"{'━'*24}\n"
                    f"© Powered by {bt}") if clean else (
                    f"{aflag} {aicon} <b>{svc}</b>  <code>{nd}</code>\n"
                    f"<i>{body160}</i>\n<i>©By <b>ADNAN</b></i>")
        return (
            f"✨ <b>{aflag} {region} {svc} OTP Received! ✨</b>\n"
            f"{'━'*28}\n"
    ...
    # ── T1: TEMPNUM ─── ENHANCED ──
        ...
        if for_group:
            return (f"{_FIRE()} {aflag} <b>{region} {svc} OTP!</b> ✨\n\n"
                    ...
                    f"<i>{body160}</i>\n<i>©By {bt}</i>")
        return (
            f"{_FIRE()} <b>{aflag} {region} {svc} OTP Received! ✨ {_FIRE()}\n\n"
            f"┌─ 🕐 <b>Time:</b> <code>{now_ts}</code>\n"
            f"├─ 🌍 <b>Country:</b> {region} {aflag}\n"
    ...
    # ── T28: SUNSET ──────────────────────────────────────────────────
        ...
        # ── T29: ULTRA PRIME (🏆 luxury crown style) ─────────────────
        if for_group:
            return (f"{_CROWN()} {aflag} {aicon} <b>#{svc}</b>  <code>{nd}</code> {_CROWN()}\n"
                    f"{_GEM()} <code>{fotp}</code>  <i>{bt}</i>") if clean else (
                    f"{_CROWN()} {aflag} {aicon} <b>#{svc}</b>  <code>{nd}</code>\n"
                    f"<i>{body160}</i>  <i>{bt}</i>")
        return (
            f"{_CROWN()}{_CROWN()} <b>🏆 ULTRA PRIME</b> {_CROWN()}{_CROWN()}\n"
            f"{'━'*22}\n"
            f"{aflag} {aicon} <b>#{svc}</b>  <code>{nd}</code>\n"
            f"{_GEM()} <b>OTP:</b>  <code>{fotp}</code> {_GEM()}\n"
            f"{'━'*22}\n"
    ...
    # ── T35: ULTRA FIRE PREMIUM ─────────────────────────────────────
    elif t == 35:
        if for_group:
            return (f"{_FIRE()} {aflag} <b>#{svc}</b>  <code>{nd}</code>\n"
                    f"{_FIRE()} <code>{fotp}</code>  <i>{bt}</i>") if clean else (
                    f"{_FIRE()} {aflag} <b>#{svc}</b>  <code>{nd}</code>\n"
                    f"<i>{body160}</i>  <i>{bt}</i>")
        return (
            f"{_FIRE()}{_FIRE()} <b>ULTRA FIRE PREMIUM</b> {_FIRE()}{_FIRE()}\n"
            f"{'━'*22}\n"
            f"{aflag} {aicon} <b>#{svc}</b>  <code>{nd}</code>\n"
            f"{_KEY()} <code>{fotp}</code>\n{'━'*22}\n"
            f"💬 <i>{body260}</i>\n{_FIRE()} <i>©By {bt}</i> {_FIRE()}"
        ) if clean else (
            f"{_FIRE()} {aflag} {aicon} <b>#{svc}</b>  <code>{nd}</code>\n"
            f"💬 <i>{body260}</i>\n{_FIRE()} <i>©By {bt}</i> {_FIRE()}")

    # ── T36: ELITE STAR PREMIUM ─────────────────────────────────────
    elif t == 36:
        if for_group:
            return (f"{_STAR()} {aflag} <b>#{svc}</b>  <code>{nd}</code>\n"
                    f"{_STAR()} <code>{fotp}</code>  <i>{bt}</i>") if clean else (
                    f"{_STAR()} {aflag} <b>#{svc}</b>  <code>{nd}</code>\n"
                    f"<i>{body160}</i>  <i>{bt}</i>")
        return (
            f"{_STAR()}{_STAR()} <b>ELITE STAR PREMIUM</b> {_STAR()}{_STAR()}\n"
            f"{'━'*22}\n"
            f"{aflag} {aicon} <b>#{svc}</b>  <code>{nd}</code>\n"
            f"{_KEY()} <code>{fotp}</code>\n{'━'*22}\n"
            f"💬 <i>{body260}</i>\n{_STAR()} <i>©By {bt}</i> {_STAR()}"
        ) if clean else (
            f"{_STAR()} {aflag} {aicon} <b>#{svc}</b>  <code>{nd}</code>\n"
            f"💬 <i>{body260}</i>\n{_STAR()} <i>©By {bt}</i> {_STAR()}")

    # ── T37: VOICE CALL PREMIUM ────────────────────────────────────
    elif t == 37:
        if for_group:
            return (f"📱 {aflag} <b>#{svc}</b>  <code>{nd}</code>\n"
                    f"📱 <code>{fotp}</code>  <i>{bt}</i>") if clean else (
    ...
    # ── T38: SECURE SERVICE PREMIUM ────────────────────────────────
    elif t == 38:
        if for_group:
            return (f"✉️ {aflag} <b>#{svc}</b>  <code>{nd}</code>\n"
                    f"✉️ <code>{fotp}</code>  <i>{bt}</i>") if clean else (
                    f"✉️ {aflag} <b>#{svc}</b>  <code>{nd}</code>\n"
                    f"<i>{body160}</i>  <i>{bt}</i>")
        return (
            f"✉️ <b>Secure Service Premium</b> ✉️\n{'─'*22}\n"
            f"{aflag} {aicon} <b>#{svc}</b>  <code>{nd}</code>\n"
            f"🔑 <code>{fotp}</code>\n{'─'*22}\n"
            f"💬 <i>{body260}</i>\n<i>©By {bt}</i>"
        ) if clean else (
    ...
    # ── T39: PRIVATE MAX SHIELD ────────────────────────────────────
    elif t == 39:
        if for_group:
            return (f"{_LOCK()} {aflag} <b>#{svc}</b>  <code>{nd}</code>\n"
                    f"{_LOCK()} <code>{fotp}</code>  <i>{bt}</i>") if clean else (
                    f"{_LOCK()} {aflag} <b>#{svc}</b>  <code>{nd}</code>\n"
                    f"<i>{body160}</i>  <i>{bt}</i>")
        return (
            f"{_LOCK()}{_LOCK()} <b>PRIVATE MAX SHIELD</b> {_LOCK()}{_LOCK()}\n"
            f"{'━'*22}\n"
            f"{aflag} {aicon} <b>#{svc}</b>  <code>{nd}</code>\n"
            f"{_KEY()} <code>{fotp}</code>\n{'━'*22}\n"
            f"💬 <i>{body260}</i>\n{_LOCK()} <i>©By {bt}</i> {_LOCK()}"
        ) if clean else (
            f"{_LOCK()} {aflag} {aicon} <b>#{svc}</b>  <code>{nd}</code>\n"
            f"💬 <i>{body260}</i>\n{_LOCK()} <i>©By {bt}</i> {_LOCK()}")
        ...
        # ── DEFAULT FALLBACK: Use Ultra Prime (theme 29) for out-of-range indices
        if for_group:
            return (f"{_CROWN()} {aflag} {aicon} <b>#{svc}</b>  <code>{nd}</code> {_CROWN()}\n"
                    f"{_GEM()} <code>{fotp}</code>  <i>{bt}</i>") if clean else (
                    f"{_CROWN()} {aflag} {aicon} <b>#{svc}</b>  <code>{nd}</code>\n"
...
def add_dynamic_design_elements(msg_text: str, theme_id: int) -> str:
    ...
    Creates more visually appealing, modern OTP GUI with dynamic effects.
    """
    # Add loading animation based on theme
    animations = {
        0: "✨",  # CRACK OTP - sparkles
        1: "⚡",  # TEMPNUM - electric
        2: "💡",  # NEON ELECTRIC - lightbulb
        3: "🌙",  # PREMIUM DARK - moon
        4: "  ",  # MINIMAL CLEAN - minimal
        5: "👑",  # ROYAL GOLD - crown
        6: "🎆",  # JACK-X - fireworks
        7: "⚔️",  # CYBER MATRIX - sword
        8: "🔥",  # FIRE STORM - fire
        9: "❄️",  # ICE BLUE - ice
        10: "🖤",  # SHADOW DARK - heart
        11: "₿",  # CRYPTO - bitcoin
        12: "🌸",  # SAKURA - flower
        13: "🎖",  # MILITARY - medal
        14: "💚",  # HACKER GREEN - green
        15: "💎",  # DIAMOND - diamond
        16: "🔥",  # SIGMA CLASSIC - fire
        17: "💓",  # PULSE - pulse
        18: "⚡",  # BOLT - electric
        19: "🌹",  # ROSE GOLD - rose
        20: "🚀",  # ASTRO - rocket
        ...
        27: "💚",  # EMERALD - green
        28: "🌅",  # SUNSET - sunrise
        29: "👑",  # ULTRA PRIME - crown
    }
    
    anim = animations.get(theme_id % 40, "✨")
    ...
    def decorator(func):
        async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            if uid in user_times:
                ...
                if elapsed < seconds:
                    await update.message.reply_text(
                        f"⏱️ <b>Please wait {seconds - int(elapsed)}s before trying again.</b>",
                        parse_mode="HTML"
                    )
                    return
...
def otp_keyboard(otp: Optional[str], full_msg: str = "",
    ...
    """
    🔐 PRIVACY-FOCUSED OTP KEYBOARD — Hidden code with tap-to-copy design.
    Shows "..........." button that copies actual OTP when clicked (secure UX).
    Different OTP GUI themes show different button sets for variety.
    Group: Compact with copy + bot/group links.
    DM: Theme-specific buttons with hidden OTP reveal.
    """
    ...
    if for_group:
        # GROUP MODE: Compact with hidden OTP copy + bot/group links (privacy focus)
        # Show only emoji + stars, when clicked OTP is copied (not shown)
        if clean:
            # Simple button: emoji + stars (plain text, no HTML), click to copy OTP
            rows.append([{"text": f"🔐 {'★' * 4}", "copy_text": {"text": clean}, "style": "success"}])
        link_row = []
        if panel_url: link_row.append(btn("🤖 BOT", url=panel_url, style="danger"))
        if info_url:  link_row.append(btn("📢 CHANNEL", url=info_url, style="primary"))
        if link_row: rows.append(link_row)
        return {"inline_keyboard": rows}

    # DM MODE: Theme-specific diverse buttons (show actual OTP code)
    
    # ── THEME 0-9: CLASSIC (Copy OTP + SMS + Bot + Group) ──
    if t < 10:
        if clean:
            # Show actual OTP code in DM for easy copying
            rows.append([{"text": f"📋  Copy OTP: {clean}", "copy_text": {"text": clean}, "style": "success"}])
        if full_msg:
            rows.append([{"text": "📩  Copy Message", "copy_text": {"text": full_msg[:256]}, "style": "primary"}])
        link_row = []
        if panel_url: link_row.append(btn("🤖  Get Numbers", url=panel_url, style="primary"))
        if info_url:  link_row.append(btn("💬  Community", url=info_url, style="success"))
        if link_row: rows.append(link_row)
    
    # ── THEME 10-19: ELECTRIC (Copy + Wallet + Developer + Settings) ──
    elif t < 20:
        if clean:
            # Show actual OTP code in button
            rows.append([{"text": f"⚡  Copy: {clean}", "copy_text": {"text": clean}, "style": "success"}])
        rows.append([btn("⚙️  Settings", cbd="user_settings", style="danger")])
        link_row = []
        if dev_url: link_row.append(btn("👨‍💻  Dev", url=dev_url, style="success"))
        if support_url: link_row.append(btn("🆘  Support", url=support_url, style="danger"))
        if link_row: rows.append(link_row)
    
    # ── THEME 20-29: PREMIUM (Copy OTP + Premium features) ──
    else:
        if clean:
            # Show actual OTP code in button
            rows.append([{"text": f"✅  Copy: {clean}", "copy_text": {"text": clean}, "style": "success"}])
        if full_msg:
            rows.append([{"text": "📄  Full Text", "copy_text": {"text": full_msg[:256]}, "style": "primary"}])
        rows.append([btn("🔗  Bot", url=panel_url or CHANNEL_LINK, style="primary"),
                     btn("👨‍💻  Creator", url=dev_url or SUPPORT_USER, style="success")])
        rows.append([btn("📖  Help", cbd="cmd_help", style="primary"),
                     btn("❓  FAQ", cbd="faq", style="danger")])
...
def main_menu_kb() -> InlineKeyboardMarkup:
    """Professional main menu with colored buttons - child bots hide bot creation."""
    rows = []
    
    # ✅ Row 1: Get Number | Create Bot (only in main bot)
    row1 = [btn("🔥  Get Number", cbd="buy_menu", style="primary")]
    if not IS_CHILD_BOT:
        row1.append(btn("🤖  Create My Bot", cbd="create_bot_menu", style="success"))
    else:
        row1.append(btn("👤  My Profile", cbd="profile", style="success"))
    rows.append(row1)
    
    # Row 2: My Stats | My History
    rows.append([btn("📊  My Stats",      cbd="mystats",       style="primary"),
                 btn("📜  My History",    cbd="myhistory",     style="success")])
    
    # Row 3: My OTPs | Premium
    rows.append([btn("💎  My OTPs",       cbd="my_otps",       style="danger"),
                 btn("💎  Premium",       cbd="premium_menu",   style="success")])
    
    # Row 4: Analytics (only main bot) | Settings
    row4 = []
    if not IS_CHILD_BOT:
        row4.append(btn("📈  Analytics", cbd="analytics", style="primary"))
    row4.append(btn("⚙️  Settings", cbd="user_settings", style="primary"))
    if row4:
        rows.append(row4)
    
    # Row 5: Tutorials
    rows.append([btn("📚  Tutorials", cbd="tutorials", style="danger")])
    
    # Links row (Channel & Community)
    link_row = []
    if CHANNEL_LINK:
        link_row.append(btn("📢  Channel", url=CHANNEL_LINK, style="primary"))
    if OTP_GROUP_LINK:
        link_row.append(btn("💬  Community", url=OTP_GROUP_LINK, style="success"))
    if link_row:
    ...
    bot_row = []
    if nb:
        bot_row.append(btn("🔗  Get Numbers", url=nb, style="primary"))
    sup = SUPPORT_USER.lstrip("@")
    if sup:
        bot_row.append(btn("🆘  Support", url=f"https://t.me/{sup}", style="danger"))
    if bot_row:
        rows.append(bot_row)
    ...
    if dev:
        rows.append([btn("👨‍💻  Developer", url=f"https://t.me/{dev}", style="success")])
    
    # Help & Back
    rows.append([btn("❓  Help", cbd="cmd_help", style="primary"),
                 btn("🏠  Back", cbd="start", style="danger")])
...
def pagination_kb(page: int, total_pages: int, action: str) -> dict:
    ...
    if total_pages > 1:
        nav_row = []
        if page > 1:
            nav_row.append(btn("⬅  Prev", cbd=f"{action}_page_{page-1}", style="primary"))
        nav_row.append(btn(f"{page}/{total_pages}", cbd="noop", style="success"))
        if page < total_pages:
            nav_row.append(btn("Next ➡", cbd=f"{action}_page_{page+1}", style="primary"))
        if nav_row:
            rows.append(nav_row)
    rows.append([btn("🔙  Back", cbd="main_menu", style="danger")])
    return {"inline_keyboard": rows}
...
def search_results_kb(results: list, search_type: str = "users") -> dict:
    ...
    for idx, item in enumerate(results[:10]):
        ...
        rows.append([btn(f"👤 {label}", cbd=f"view_{search_type}_{idx}", style="primary")])
    if len(results) > 10:
        rows.append([btn(f"📊 +{len(results)-10} more...", cbd="noop", style="success")])
    rows.append([btn("🔙  Back", cbd="admin_home", style="danger")])
    return {"inline_keyboard": rows}
...
def admin_main_kb(perms: list, is_sup: bool) -> dict:
    ...
    # NUMBERS & BROADCAST
    r = []
    if "manage_files" in perms: r.append(btn("📂  Numbers",    cbd="admin_numbers",      style="primary"))
    if "broadcast"    in perms: r.append(btn("📢  Broadcast",  cbd="admin_broadcast",    style="danger"))
    if r: rows.append(r)
    
    # STATISTICS & ADVANCED ANALYTICS
    r = []
    if "view_stats" in perms: r.append(btn("📊  Statistics", cbd="admin_stats_menu",   style="success"))
    if "view_stats" in perms: r.append(btn("📈  Advanced",    cbd="advanced_analytics", style="primary"))
    if r: rows.append(r)
    
    # USERS (full-width button - important)
    r = []
    if is_sup and not disable_bot_mgmt: r.append(btn("👤  Users",       cbd="admin_users",        style="success"))
    if r: rows.append(r)
    
    # PANELS & LOG GROUPS
    r = []
    if "manage_panels" in perms: r.append(btn("🔌  Panels",     cbd="admin_panel_manager",style="primary"))
    if "manage_logs"   in perms: r.append(btn("📋  Log Groups", cbd="admin_manage_logs", style="primary"))
    if r: rows.append(r)
    
    # CLEAR LOGS (full-width button - important action)
    r = []
    if "manage_logs" in perms: r.append(btn("🗑  Clear Logs",  cbd="clear_all_logs",   style="danger"))
    if r: rows.append(r)
    
    # ADMINS & PERMISSIONS
    r = []
    if is_sup and not disable_bot_mgmt: r.append(btn("👥  Admins",        cbd="admin_manage_admins", style="danger"))
    if is_sup and not disable_bot_mgmt: r.append(btn("🔐  Permissions",   cbd="admin_perms",         style="success"))
    if r: rows.append(r)
    
    # TUTORIALS & SETTINGS
    r = []
    if is_sup and not disable_bot_mgmt: r.append(btn("📚  Tutorials",     cbd="admin_tutorials",     style="primary"))
    r.append(btn("⚙️  Settings",                  cbd="admin_settings",      style="primary"))
    if r: rows.append(r)
    
    # FETCH SMS & CHILD BOTS (hidden in child bots)
    r = []
    if "manage_panels" in perms: r.append(btn("📡  Fetch SMS",  cbd="admin_fetch_sms",   style="success"))
    if is_sup and not disable_bot_mgmt: r.append(btn("🤖  Child Bots", cbd="admin_bots",        style="danger"))
    if r: rows.append(r)
    
    # SYSTEM (full-width button - important, hidden in child bots)
    r = []
    if is_sup and not disable_bot_mgmt: r.append(btn("🔧  System",        cbd="admin_system",        style="primary"))
    if r: rows.append(r)
    
    # HOME & EXIT
    rows.append([btn("🏠  Home", cbd="main_menu", style="success"), btn("❌  Exit", cbd="cancel", style="danger")])
...
def confirm_del_panel_kb() -> dict:
    return {"inline_keyboard": [[
        btn("✅  Yes, Delete", cbd="p_del_confirm", style="danger"),
        btn("❌  Cancel",      cbd="admin_panel_manager",  style="success"),
    ]]}
...
def admin_settings_kb() -> dict:
    theme_name = _THEME_NAMES.get(OTP_GUI_THEME % 40, "Unknown")
    return {"inline_keyboard": [
...
def gui_theme_kb(page: int = 0) -> dict:
    """40 OTP designs, 8 per page, colored buttons. Active theme = green."""
    per_page    = 8
    start       = page * per_page
    end         = min(start + per_page, 40)
    total_pages = (40 + per_page - 1) // per_page
    rows = []; row = []
    for tid in range(start, end):
        active = tid == OTP_GUI_THEME % 40
        mark   = "✅ " if active else ""
...
def gui_theme_page_kb(page: int = 0) -> InlineKeyboardMarkup:
    """40 compact designs, 10 per page. Super admins only."""
    kb = []
    start_t = page * 10
    end_t   = min(start_t + 10, 40)
    for tid in range(start_t, end_t):
        name = _THEME_NAMES.get(tid, f"Design {tid+1}")
        mark = "✅ " if tid == OTP_GUI_THEME % 40 else ""
    ...
    nav.append(InlineKeyboardButton(f"  {page+1}/4  ", callback_data="ignore", style="primary"))
    if page < 3:   nav.append(InlineKeyboardButton("Next ▶", callback_data=f"gui_page_{page+1}", style="success"))
...
def perms_kb(selected: list, uid: int) -> dict:
    ...
    for p, label in PERMISSIONS.items():
        ...
        rows.append([btn(f"{mark}  {label}", cbd=f"ptoggle|{uid}|{p}",
                         style="success" if active else "primary")])
    rows.append([btn("💾  Save", cbd=f"pdone|{uid}", style="success"),
...
async def _finalize_panel_edit(uid: int, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("✅ <b>Panel updated!</b>", parse_mode="HTML")
    except Exception as e:
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
    "💎 <b>{bot_label} {bot_name}</b>{role_line}\n\n"
    "⚡ <b>Features:</b> Real-time OTP • Auto-assign Numbers • 200+ Countries • Advanced Analytics\n\n"
    "👇 <b>Get Started</b>"
).format(bot_label=bot_label, bot_name=bot_name, role_line=role_line)
...
async def cmd_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if IS_CHILD_BOT and sup:
        await update.message.reply_text(
            "⚠️ <b>Not Available in Child Bot</b>\n\n"
    ...
    lines = [
        "<b>🛡️ ADMIN PANEL</b>",
        f"👤 {role} • ID: <code>{uid}</code>",
        "",
        f"📱 Numbers: <b>{stats.get('available',0)}</b>",
        f"🔌 Panels: <b>{run_cnt}/{panel_cnt}</b> online",
    ]
    if not IS_CHILD_BOT:
        bot_cnt = len(bm.list_bots())
        lines.append(f"🤖 Bots: <b>{bot_cnt}</b>")
    await update.message.reply_text(
        "\n".join(lines),
...
async def cmd_mystats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    achievement_badges = []
    if otps_count >= 1: achievement_badges.append("🌟 First OTP")
    if otps_count >= 10: achievement_badges.append("⚡ OTP Collector")
    if otps_count >= 100: achievement_badges.append("🔥 OTP Master")
    if len(active_nums) >= 3: achievement_badges.append("📱 Multi-User")
...
async def cmd_myhistory(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb_rows = []
    if has_prev:
        kb_rows.append([btn("⬅  Prev", cbd=f"cmd_history_{page-1}", style="primary")])
    if has_next:
        kb_rows.append([btn("Next ➡", cbd=f"cmd_history_{page+1}", style="primary")])
    kb_rows.append([btn("🔙  Back", cbd="main_menu", style="danger")])
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
                ...
                await update.message.reply_text(
                    ...
                    reply_markup=InlineKeyboardMarkup([[
                        InlineKeyboardButton(f"📋 Copy: {found_otp}",
                                             copy_text=CopyTextButton(text=found_otp), style="success"),
                        InlineKeyboardButton("🔙 Back",callback_data="admin_otp_tools", style="danger")]]),
                    parse_mode="HTML")
            else:
                await update.message.reply_text(
    ...
    if bcast_mode == "text" and context.user_data.get("awaiting_broadcast"):
        ...
        await context.bot.send_message(uid,
            ...
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back",callback_data="admin_broadcast_menu", style="danger")]]))
        return
    ...
    if bcast_mode in ("image", "video", "tutorial") and context.user_data.get("awaiting_broadcast"):
        ...
        styled_caption = f"{'{'}{bcast_mode.upper()}{'}'} <b>Broadcast</b>\n{'━'*24}\n{bcast_caption}\n\n<i>— Admin Update</i>"
    ...
    if context.user_data.get("awaiting_broadcast_caption"):
        ...
        styled_caption = f"{'🎬' if bcast_mode=='video' else '🖼️' if bcast_mode=='image' else '📚'} <b>Broadcast</b>\n{'━'*24}\n{caption}\n\n<i>— Admin Update</i>"
        
        for target in all_users:
            try:
                if bcast_mode in ("image","video") and ("bcast_photo" in context.user_data or "bcast_video_id" in context.user_data):
                    if bcast_mode == "image":
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("us_"):
        ...
        if action=="done":
            ...
            await query.edit_message_text(
                ...
                f"📱 Services: {', '.join(sel)}\n🌍 {flag} {country}",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back",callback_data="admin_files", style="primary")]]),
                parse_mode="HTML")
    ...
    if data=="send_test_otp":
        ...
        await query.edit_message_text("✅ Test OTP sent to your DM.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back",callback_data="admin_notify_menu", style="primary")]]), parse_mode="HTML")
        return
    ...
    if data.startswith("set_gui_theme_"):
        ...
        if not is_super_admin(uid):
            ...
            return
        OTP_GUI_THEME = int(data.split("_")[-1]) % 40
    ...
    if data=="admin_fetch_sms":
        ...
        for p in PANELS:
            ...
            if sms is None: report+=f"❌ <b>{html.escape(p.name)}</b>: Auth failed.\n\n"
                ...
                report+="\n"
        bkb=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back",callback_data="admin_home", style="primary")]])
        if len(report)>4000:
    ...
    if not IS_CHILD_BOT:
        if data=="admin_bots":
            ...
            if bots:
                lines_txt="\n".join(f"{'🟢' if b.get('running') else '🔴'} <b>{html.escape(b['name'])}</b>  <code>{b['id']}</code>" for b in bots)
                msg_txt=f"🖥  <b>Bot Manager</b>\n{D}\nTotal: <b>{len(bots)}</b>  |  Running: <b>{tr}</b>\n\n{lines_txt}"
            else:
    ...
    if data=="pick_gui" or data.startswith("gui_set_"):
        ...
        await safe_edit(query,
            ...
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🎨  Open Theme Picker", callback_data="admin_gui_theme", style="success"),
    ...
    if data=="cbot_have_panel":
        ...
        await safe_edit(query,
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🆓 Free",  callback_data="cbot_tier_free", style="success")],
                [InlineKeyboardButton("💎 Pro ($5/mo)",  callback_data="cbot_tier_pro", style="primary")],
                [InlineKeyboardButton("🏆 Enterprise ($10/mo)",  callback_data="cbot_tier_enterprise", style="danger")],
                [InlineKeyboardButton("🔙  Back", callback_data="create_bot_menu", style="success")],
            ]))
        return
    ...
    if data.startswith("cbot_tier_"):
        ...
        await safe_edit(query,
            ...
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🔙  Back", callback_data="cbot_have_panel", style="danger")]]))
        return
        ...
        try:
            ...
            if not has_panel:
                ...
                await query.edit_message_text(
                    ...
                    parse_mode="HTML",
                    reply_markup=InlineKeyboardMarkup([[
                        InlineKeyboardButton("🏠  Back to Home", callback_data="main_menu", style="success")
                    ]]))
                return
    ...
    if data.startswith("bot_confirm_"):
        ...
        async with db.async_session() as session:
            existing = await session.execute(
                select(ChildBot).where(ChildBot.token == token_input)
            )
            if existing.scalar():
                await query.edit_message_text(
    ...
    if data.startswith("bot_edit_"):
        ...
        await query.edit_message_text(
            ...
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("1️⃣  Bot Name", callback_data=f"bot_edit_field_bot_name_{uid}")],
                [InlineKeyboardButton("2️⃣  Bot Token", callback_data=f"bot_edit_field_token_{uid}")],
                [InlineKeyboardButton("3️⃣  Bot Username", callback_data=f"bot_edit_field_bot_username_{uid}")],
                [InlineKeyboardButton("4️⃣  Admin ID", callback_data=f"bot_edit_field_admin_id_{uid}")],
                [InlineKeyboardButton("5️⃣  Channel Link", callback_data=f"bot_edit_field_channel_{uid}")],
                [InlineKeyboardButton("6️⃣  OTP Group Link", callback_data=f"bot_edit_field_otp_group_{uid}")],
                [InlineKeyboardButton("7️⃣  Number Bot Link", callback_data=f"bot_edit_field_number_bot_{uid}")],
                [InlineKeyboardButton("8️⃣  Support Username", callback_data=f"bot_edit_field_support_{uid}")],
                [InlineKeyboardButton("9️⃣  Group Chat ID", callback_data=f"bot_edit_field_group_id_{uid}")],
    ...
    if data=="user_settings":
        ...
        await query.edit_message_text(
            "<b>⚙️  User Settings</b>\n\n"
            "User settings allow you to customize your experience.\n\n"
            "Features coming soon:\n"
            "• Notification preferences\n"
            "• Number auto-assign settings\n"
            "• OTP display formats\n\n"
    ...
    if data=="tutorials":
        ...
        kb_rows.append([InlineKeyboardButton("🏠  Back", callback_data="main_menu", style="danger")])
    ...
    if data.startswith("tut_view_"):
        ...
        if tut.content_type in ("video", "both") and tut.video_file_id:
            kb_rows.append([InlineKeyboardButton("▶️  Watch Video", callback_data=f"tut_video_{tut.id}", style="success")])
        
        kb_rows.append([InlineKeyboardButton("📚  Back to Tutorials", callback_data="tutorials", style="primary")])
        kb_rows.append([InlineKeyboardButton("🏠  Back to Menu", callback_data="main_menu", style="danger")])
    ...
    if data=="my_otps":
        ...
        if not nums:
            await query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("🔥  Get Number", callback_data="buy_menu", style="primary"),
                    InlineKeyboardButton("🏠  Back", callback_data="main_menu", style="danger")
                ]]),
                parse_mode="HTML")
            return
    ...
    if data.startswith("otp_detail_"):
        ...
        kb_rows = [
            [InlineKeyboardButton("📱  My OTPs", callback_data="my_otps", style="primary")],
            [InlineKeyboardButton("🏠  Back", callback_data="main_menu", style="danger")]
        ]
    ...
    if data=="premium_menu":
        ...
        await query.edit_message_text(
            ...
            "Unlock exclusive pro-level features:\n\n"
            "🔥 <b>Professional OTP Rendering</b>\n"
            "   • 30 unique GUI themes\n"
            "   • Custom button combinations\n\n"
            "📊 <b>Enhanced Analytics</b>\n"
            "   • Real-time statistics\n"
            "   • Detailed reports\n\n"
            "🚀 <b>Advanced Features</b>\n"
            "   • Unlimited panels\n"
            "   • Priority support\n\n"
            "<i>Premium access is coming soon!</i>",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🏠  Back", callback_data="main_menu", style="success")
            ]]), parse_mode="HTML")
        return
    ...
    if data=="cmd_help":
        ...
        await query.edit_message_text(
            ...
            "A: Yes, 100% free!",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("💬  Support", url=f"https://t.me/{SUPPORT_USER.lstrip('@')}" if SUPPORT_USER else "https://t.me", style="primary"),
                InlineKeyboardButton("🏠  Back", callback_data="main_menu", style="danger")
            ]]), parse_mode="HTML")
        return
    ...
    if data=="admin_tutorials":
        ...
        kb_rows.append([InlineKeyboardButton("🔙  Back", callback_data="admin_home", style="danger")])
    ...
    if data.startswith("admin_tut_"):
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("📚  Back", callback_data="admin_tutorials", style="danger")]
            ]),
            parse_mode="HTML")
        return
    ...
    if data=="admin_settings":
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🏠  Back", callback_data="admin_home", style="danger")
            ]]), parse_mode="HTML")
        return
    ...
    if data=="cancel":
        ...
        if uid in AWAITING_ADMIN_ID:
            del AWAITING_ADMIN_ID[uid]
        await query.edit_message_text("✅ Cancelled. Return to main menu.", reply_markup=InlineKeyboardMarkup([[
    ...
    if data=="admin_broadcast_menu":
        ...
        await safe_edit(query,
            ...
            reply_markup=InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("👁️  View Templates", callback_data="bcast_templates", style="success")],
                [InlineKeyboardButton("🔙  Back", callback_data="admin_settings", style="danger")],
            ]))
        return
    ...
    if data=="bcast_templates":
        ...
        await safe_edit(query, templates,
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔙  Back", callback_data="admin_broadcast_menu", style="danger")]
            ]))
        return
