...
def start_bot(bid: str) -> tuple:
    ...
    # ✅ CRITICAL: Clean up any existing instances
    # 1. If registry has a PID and process is running, stop it
    if is_running(bid):
        try:
            stop_bot(bid)
            time.sleep(2)
    ...
    # 2. Cleanup orphaned processes (best-effort on all platforms)
    try:
        # Try to find and kill Python processes in this folder
        try:
            import psutil
            pid_list = [p.pid for p in psutil.process_iter(['pid', 'name', 'cwd']) 
                       if 'python' in p.name.lower() and folder in str(p.cwd())]
            for pid in pid_list:
                try:
            ...
            # psutil not available, try shell commands
            if sys.platform != "win32":
                os.system(f"pkill -9 -f 'python.*bot.py' 2>/dev/null || true")
    except:
    ...
    try:
        ...
        save_registry(reg)
        time.sleep(2)  # Give process time to fully initialize
